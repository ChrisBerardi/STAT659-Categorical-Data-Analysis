...
title'Variable Selection Model';
...
;
*forward selection;
*Use p=.2 for all inclusion in model;
title'forward';
proc logistic desc; 
model sta=age sex race ser can crn inf cpr sys hra pre 
typ fra po2 ph pco bic cre loc/ selection=forward slentry=.2;
run;

title'stepwise';
proc logistic desc; 
model sta=age sex race ser can crn inf cpr sys hra pre 
typ fra po2 ph pco bic cre loc/ selection=stepwise slstay=.2 slentry=.2;
run;

title'backwards';
proc logistic desc; 
model sta=age sex race ser can crn inf cpr sys hra pre 
typ fra po2 ph pco bic cre loc/ selection=backwards slstay=.2;
run;
...
*Use p=.2 for all inclusion in model;
title'univariates';
proc logistic desc; model sta=age; run;
proc logistic desc; model sta=sex; run;
proc logistic desc; model sta=race; run;
proc logistic desc; model sta=ser; run;
proc logistic desc; model sta=can; run;
proc logistic desc; model sta=crn; run;
proc logistic desc; model sta=inf; run;
proc logistic desc; model sta=cpr; run;
proc logistic desc; model sta=sys; run;
proc logistic desc; model sta=hra; run;
proc logistic desc; model sta=pre; run;
proc logistic desc; model sta=typ; run;
proc logistic desc; model sta=fra; run;
proc logistic desc; model sta=po2; run;
proc logistic desc; model sta=ph; run;
proc logistic desc; model sta=pco; run;
proc logistic desc; model sta=bic; run;
proc logistic desc; model sta=cre; run;
proc logistic desc; model sta=loc; run;

*try the significan terms;
title'Main Effect Models';
proc logistic desc;
model sta = age ser crn inf cpr sys typ cre loc can fra pco/ covb;
run;

*remove least significant at each level;

proc logistic desc;
model sta = age ser crn inf cpr sys typ loc can fra pco;
run;

proc logistic desc;
model sta = age ser crn cpr sys typ loc can fra pco;
run;

proc logistic desc;
model sta = age ser crn sys typ loc can fra pco;
run;

proc logistic desc;
model sta = age ser sys typ loc can fra pco;
run;

proc logistic desc;
model sta = age ser sys typ loc can pco;
run;

proc logistic desc;
model sta = age sys typ loc can pco;
run;

*now check the interactions individually;
title'Interaction Models';
proc logistic desc;
model sta = age sys typ loc can pco age*sys;
run;

proc logistic desc;
model sta = age sys typ loc can pco age*typ;
run;

proc logistic desc;
model sta = age sys typ loc can pco age*loc;
run;

proc logistic desc;
model sta = age sys typ loc can pco age*can;
run;

proc logistic desc;
model sta = age sys typ loc can pco age*pco;
run;

proc logistic desc;
model sta = age sys typ loc can pco sys*typ;
run;

proc logistic desc;
model sta = age sys typ loc can pco sys*loc;
run;

proc logistic desc;
model sta = age sys typ loc can pco sys*can;
run;

proc logistic desc;
model sta = age sys typ loc can pco sys*pco;
run;

proc logistic desc;
model sta = age sys typ loc can pco typ*loc;
run;

proc logistic desc;
model sta = age sys typ loc can pco typ*can;
run;

proc logistic desc;
model sta = age sys typ loc can pco typ*pco;
run;

proc logistic desc;
model sta = age sys typ loc can pco loc*can;
run;

proc logistic desc;
model sta = age sys typ loc can pco loc*pco;
run;

proc logistic desc;
model sta = age sys typ loc  pco can*age;
run;

*try the full model;
title'Full Model';
proc logistic desc;
model sta = age sys typ loc can pco age*sys age*loc sys*loc age*can/ lackfit scale=none aggregate influence covb;
output out=pvals p=p;
run;

title'Full Model';
ods graphics on;
proc logistic desc plots(label)=leverage influence;
model sta = age sys typ loc pco age*sys age*loc sys*loc age*can/ lackfit scale=none aggregate covb;
output out=pvals p=p;
run;
ods graphics off;

%marginal(independents= age sys, dependent=sta, predicted=p);
